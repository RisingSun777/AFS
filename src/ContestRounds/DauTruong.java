...
public class DauTruong implements KeyListener {
	...
	private void drawFirstCol(Graphics2D g2d) {
		...
		for(int i = 0; i < 3; i++) {
			...
			g2d.translate(0, 15);
			GraphicsUtil.drawString(g2d, (i + 1)*10 + " điểm", temp_rect_shad, GraphicsUtil.Align.North);
			g2d.setColor(Color.WHITE);
			GraphicsUtil.drawString(g2d, (i + 1)*10 + " điểm", temp_rect, GraphicsUtil.Align.North);
			g2d.setTransform(new AffineTransform());
			g2d.draw(temp_rect);
		}
	}
	...
	public void keyPressed(KeyEvent e) {
		...
		case KeyEvent.VK_ENTER:
			...
			if(!list.get(selection.x).get(selection.y).isvisible)
				loadQuestion(list.get(selection.x).get(selection.y));
			break;
		}
	}
	...
	private void loadQuestion(SelectionRectangle rect) {
		int number = DauTruong.horizontallength*rect.index.x + rect.index.y + 1;
		MainRound.current_question = number;
	}
}
...
class SelectionRectangle {
	...
	public void drawNumber(Graphics2D g2d) {
		if(isvisible) {
			...
			Integer number = DauTruong.horizontallength*index.x + index.y + 1;
